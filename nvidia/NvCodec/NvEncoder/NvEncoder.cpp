...
bool NvEncoder::isSupport(GUID codec)
{
	if (!m_hEncoder)
	{
		return false;
	...
	uint32_t count = 0;
	NV_ENCODE_API_FUNCTION_LIST *m_pEncodeAPI = &m_nvenc;
	if (m_pEncodeAPI == NULL)
	{
		return false;
	}

	m_pEncodeAPI->nvEncGetEncodeGUIDCount(m_hEncoder, &count);
	GUID guids[10];
	if (count > 10)
	{
		count = 10;
	...
	uint32_t size = count;
	m_pEncodeAPI->nvEncGetEncodeGUIDs(m_hEncoder, guids, size, &count);
